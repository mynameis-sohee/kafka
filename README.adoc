...
5. Update the Sink connector to use `UPSERT` mode => error:: kafka-connect No resolvable bootstrap urls given in bootstrap.servers 로 인해서 해당 포트에 연결이 안됨... curl localhost:8083 실행 불가
+
[source,javascript]
----
curl -X PUT http://localhost:8083/connectors/sink-jdbc-mysql-01/config \
